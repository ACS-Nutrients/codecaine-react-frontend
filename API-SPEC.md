...
## 🖼️ 이미지 분석 (Image Analysis Service - AWS Textract)
...
- cognito_id: string
- ans_current_id: number (optional, 기존 영양제 업데이트 시)
```
**Response:**
```json
{
    ...
    "nutrients": [
      {
        ...
        "amount": 1000,
        "unit": "mg",
        "daily_value_percent": 1000
      },
      {
        "name": "아연",
        "amount": 15,
        "unit": "mg",
        "daily_value_percent": 136
      }
    ]
  },
  "raw_text": "전체 추출된 텍스트...",
...
```
**사용 페이지:** `MyPage.tsx` (영양제 스캔 기능)

**처리 흐름:**
1. 프론트엔드에서 이미지 파일 업로드
2. 백엔드에서 S3에 이미지 저장
3. AWS Textract API 호출하여 텍스트 추출
4. 추출된 텍스트를 파싱하여 영양성분 정보 구조화
5. 구조화된 데이터를 DB에 저장 (analysis_supplements 테이블)
6. 프론트엔드에 결과 반환
...
- 5. 현재 복용 중인 영양제 목록 조회
- 15. 영양제 복용 기록 조회
- ✅ **21. 영양성분표 이미지 분석 (AWS Textract)** (`MyPage.tsx`)
...
6. **AWS Textract** - 이미지 분석 시 S3 버킷 권한 및 Textract IAM 역할 필요
